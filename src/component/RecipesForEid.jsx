...
export default function RecipesForEid() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    // Fetch only Eid recipes (backend expects query key `inEid=true`)
    getRecipes(undefined, "appetizers", undefined, false, true) // ✅ inEid=true ({ramadan, is inEid})
      .then((data) => {
        if (!mounted) return;
        setRecipes(data);
      })
      .catch(() => mounted && setError("Failed to load Eid recipes"))
    ...
    <div style={{ backgroundColor: "#f7fbff", minHeight: "100vh" }}>
      <div className="container py-4">
        ...
        {!loading && !error && recipes.length === 0 && (
          <div style={{ color: "#333" }}>No Eid recipes found.</div>
        )}
